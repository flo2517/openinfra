...
`tests/e2e/suites/10-multi-provider-concurrency.sh` detects this profile automatically: with it enabled, two of its three extra Provider Agents are the real containerized `provider-agent-2`/`provider-agent-3` services above (instead of another disposable host process), and with the full three-validator committee also up it additionally proves `pallet-network-validator`'s `MinQuorum` actually closing a round -- a genuine multi-node scenario the default single-validator stack cannot exercise. See `tests/e2e/AGENTS.md`.

### Fault injection: torn connections (`COMPOSE_PROFILES=chaos`)

A second opt-in profile, `chaos`, starts [toxiproxy](https://github.com/Shopify/toxiproxy) in front of PostgreSQL (`deployments/docker-compose.yml`), used by `tests/e2e/suites/20-chaos-injection.sh`'s `scenario_postgres_torn_connection` to inject a real TCP RST into an already-established Postgres connection -- a genuinely torn mid-flight failure, not the clean `docker compose stop`/`start` its other Postgres/Redis scenarios use. It is never started by plain `make dev-up` or by the `multi-node` profile, and nothing outside that one suite's own disposable extra `controlplane` worker ever talks through it -- the always-on Compose control-plane's own Postgres connection is untouched. The suite manages its own lifecycle (`docker compose --profile chaos up -d --wait toxiproxy` / `stop toxiproxy`); there is no need to pass `COMPOSE_PROFILES=chaos` to `make dev-up` yourself unless you want to poke the proxy's admin API (`http://127.0.0.1:8474` by default) directly.
