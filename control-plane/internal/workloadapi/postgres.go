...
func (r *PostgresRepository) NextPending(ctx context.Context) (Workload, error) {
	return scanWorkload(r.pool.QueryRow(ctx, selectWorkload+` WHERE state IN ('REQUESTED','SCHEDULING','LEASE_PENDING','LEASED','DEPLOYING') AND (next_attempt_at IS NULL OR next_attempt_at <= now()) ORDER BY created_at LIMIT 1`))
}
...
func (r *PostgresRepository) RetryLater(ctx context.Context, workloadID, code, message string, delay time.Duration) error {
	...
	_, err := r.pool.Exec(ctx, `UPDATE workloads SET attempt_count=attempt_count+1, next_attempt_at=now()+$2::interval, error_code=$3, last_error=$4, updated_at=now() WHERE workload_id=$1 AND state IN ('REQUESTED','SCHEDULING','LEASE_PENDING','LEASED','DEPLOYING')`, workloadID, delay.String(), code, message)
	return err
}

const selectWorkload = `SELECT workload_id::text, request_id::text, request_hash, definition, COALESCE(resource_hash,'\x'::bytea), image, state, COALESCE(provider_id,''), COALESCE(lease_id::text,''), COALESCE(container_id,''), COALESCE(error_code,''), created_at, updated_at FROM workloads`
...
func scanWorkload(row scanner) (Workload, error) {
	...
	err := row.Scan(&w.WorkloadID, &w.RequestID, &hash, &w.Definition, &resourceHash, &w.Image, &w.State, &w.ProviderID, &w.LeaseID, &w.ContainerID, &w.ErrorCode, &w.CreatedAt, &w.UpdatedAt)
	if errors.Is(err, pgx.ErrNoRows) {
