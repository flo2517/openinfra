...
| `30-migrations-rollback.sh` | `control-plane/migrations/ROLLBACK.md` is complete (a section per migration file) and correct: forward-apply -> rollback -> forward-apply again reaches the same schema, against a disposable scratch database. Postgres-only (calls neither `ensure_shared_binaries` nor anything Substrate/Provider-Agent-shaped), so it's the one suite wired into CI (`.github/workflows/ci.yml`'s `e2e-migrations` job, issue #139) -- see "What is out of scope here, and why" below for the other suites' CI status. |
| `40-content-addressed-frontend.sh` | ADR-037/issue #35 against the real stack: a fixed release tree actually pinned via the self-hosted `ipfs` (kubo) Compose service and actually served back byte-for-byte by its gateway; `control-plane/cmd/frontendrelease`'s keygen/manifest/sign/verify/publish chain producing a real signed release row; `GET /.well-known/openinfra-frontend` serving that exact release; and the dashboard's CORS/origin-allowlist honoring the just-published release's `allowed_login_origins` while rejecting an unrelated origin -- ADR-037 §11(a)/(b)/(d). §11(c) (a third-party public mirror gateway) and §11(e) (a real browser's CSP `<meta>` enforcement) are out of this suite's reach (external network egress and a real browser engine respectively) and are named, not silently skipped, in the suite's own header comment. |
...
- **CI, beyond `30-migrations-rollback.sh`.** `.github/workflows/ci.yml`'s
  ...
  postgres container the same way). The other suites still need the
