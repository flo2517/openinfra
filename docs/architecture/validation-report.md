...
The stop lifecycle is also authoritative and idempotent. A real mTLS `StopWorkload` moved the persisted workload to `STOPPING`; the Agent stopped and inspected the exact mapped container, after which the bridge finalized lease `1` as `Completed` and PostgreSQL committed `STOPPED`. Replaying the same stop request returned `STOPPED` without a second container or lease transition.
