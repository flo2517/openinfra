...
| `GET /api/v1/validator/rounds/{provider_id}` *(added after acceptance)* | Public | The open-round counterpart to `validator-scores`: pallet-network-validator's `Evidence` NMap plus the recomputed committee. Public for the same reason `validator-scores` is — committee assignment is publicly computable by design (ADR-011 §1) and every submission is an attributable extrinsic already readable by anyone with a chain connection. Serves the integer claim and its `payload_hash`, never a payload. |
| `GET /api/v1/validator/health` *(added after acceptance)* | Public | `ActiveValidatorSet` joined with each account's `Validators` record. `/api/v1/overview` already publishes the active set's accounts; this adds the lifecycle/stake state stored alongside them, which is equally public storage. |
...
| Validator submissions (`validator`, `score_bps`, `sample_count`, `payload_hash`) and validator records (`status`, `stake`, `registered_at`) | Yes, public, via the two validator views added after acceptance | No — all of it is public consensus state by construction. The evidence *payload* is the only sensitive artefact and it is never on-chain: `payload_hash` addresses it off-chain (ADR-011 §3), and no dashboard endpoint resolves that address. Accounts are rendered in full here rather than abbreviated, which is a usability choice over already-public identifiers, not a disclosure. |
